...
pub(crate) struct GspSeqRun {
    pub seq_info: fw::rpc_run_cpu_sequencer_v17_00,
}

pub(crate) trait GspSeqCmdRunner {
    fn run<'a>(&self, sequencer: &GspSequencer<'a>) ->  Result;
}

impl GspSeqCmdRunner for fw::GSP_SEQ_BUF_PAYLOAD_REG_WRITE {
    // TODO: Can we return Result instead of  Result?
    fn run<'a>(&self, sequencer: &GspSequencer<'a>) ->  Result {
        pr_info!("RegWrite: addr=0x{:x}, val=0x{:x}\n", self.addr, self.val);
        let addr = self.addr as usize;
        let val = self.val;
        with_bar!(&sequencer.bar, |b| {
            b.try_write32(val, addr);
        })
    }
}
...
impl GspSeqCmdRunner for fw::GSP_SEQ_BUF_PAYLOAD_REG_MODIFY {
    fn run<'a>(&self, sequencer: &GspSequencer<'a>) ->  Result {
        ...
        with_bar!(&sequencer.bar, |b| {
            let addr = self.addr as usize;
            if let Ok(temp) = b.try_read32(addr) {
                b.try_write32((temp & !self.mask) | self.val, addr);
            }
        })
    }
}
...
impl GspSeqCmdRunner for fw::GSP_SEQ_BUF_PAYLOAD_DELAY_US {
    fn run<'a>(&self, _sequencer: &GspSequencer<'a>) ->  Result {
...
impl GspSeqCmdRunner for fw::GSP_SEQ_BUF_PAYLOAD_REG_STORE {
    fn run<'a>(&self, sequencer: &GspSequencer<'a>) ->  Result {
        ...
        let val = with_bar!(&sequencer.bar, |b| b.try_read32(addr))?;
...
impl GspSeqCmdRunner for GspSeqCmd {
    fn run<'a>(&self, seq: &GspSequencer<'a>) ->  Result {
        match self {
            ...
            GspSeqCmd::CoreResume => {
                ...
                seq.gsp_falcon.check_reload_completed(seq.bar, Duration::from_secs(2))?;
...
impl<'a> IntoIterator for &'a GspSequencer<'a> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        // Calculate offset to command buffer
        let seq_info_size = size_of::<fw::rpc_run_cpu_sequencer_v17_00>();
        let cmd_data = &self.data[seq_info_size..];

        GspSeqIter {
            cmd_data,
            current_offset: 0,
            total_cmds: self.run.seq_info.cmdIndex,
            cmds_processed: 0,
        }
    }
}
...
impl<'a> GspSequencer<'a> {
    pub(crate) fn new(
        ...
        let seq_info_size = size_of::<fw::rpc_run_cpu_sequencer_v17_00>();
        if data.len() < seq_info_size {
            return Err(EINVAL);
        }

        // Extract sequence info from the data
        let seq_info_data = &data[..seq_info_size];
        let seq_info = fw::rpc_run_cpu_sequencer_v17_00::from_bytes(seq_info_data)?;
        let run = GspSeqRun { seq_info };
