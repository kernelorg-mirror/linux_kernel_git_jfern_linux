...
impl Bar {
    ...
    pub(crate) fn new(instmem: &Arc<InstMem>, gsp: &Arc<dyn GspManager>, bar1_size: u64,
        ...
        match bar2_size {
            Some(bar2_size) => {
                let mut bar2 = BarN::new(InstObj::new(instmem, 0x1000, 0, false, true)?, true)?;

                pr_info!("BAR 2 INIT VMM {:#x}\n", bar2_size / 2);
                let bar2_vmm = Vmm::new(instmem, Some((bar2_inner_lock_class, bar2_pd_lock_class)), 0, bar2_size / 2, NVKM_VMM_TYPE_UNMANAGED, true, true, None, Some(&mut bar2.inst), false, gsp.get_bar_pdb(2), c_str!("bar2"))?;

                gsp.update_bar_pde(1, bar2_vmm.getpd0_addr()?, 47)?;

                let obj = VramObj::wrap(0, PAGE_SIZE)?;

                let mut iobj = VramObj::vram_kmap(obj, instmem)?;
        ...
        let obj = InstObj::new(instmem, 0x1000, 0, false, true)?;
        let mut bar1 = BarN::new(obj, false)?;

        pr_info!("BAR 1 INIT VMM {:#x}\n", bar1_size);
        let vramobj = InstObj::wrap(instmem, VramObj::wrap(gsp.get_bar_pdb(1) as usize, 0x1000)?)?;
        let bar1_vmm = Vmm::new(instmem, Some((bar1_inner_lock_class, bar1_pd_lock_class)), 0, bar1_size, NVKM_VMM_TYPE_UNMANAGED, true, false, Some(vramobj), Some(&mut bar1.inst), false, 0, c_str!("bar1"))?;
