...
stress_ng_generate_args() {
	...
	# Print final stressor distribution
	echo "Configuring stress-ng with the following stressors:"
	if test "$cpu_count" -gt 0; then
		echo "- CPU stressors (matrixprod): $cpu_count"
	fi
	if test "$dir_count" -gt 0; then
		echo "- Directory stressors: $dir_count"
	fi
	if test "$dirdeep_count" -gt 0; then
		echo "- Deep directory stressors: $dirdeep_count"
	fi
	if test "$dirmany_count" -gt 0; then
		echo "- Many directory stressors: $dirmany_count"
	fi
	echo
