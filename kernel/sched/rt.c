...
static void adjust_cg_throt_update(struct rt_rq *rt_rq, bool throt)
{
	 ...
	 * We need to propagate it up the tree starting from rt_rq's parent rt_rq.
	 */
	for (; tg; tg = tg->parent) {
		bool after;
		bool before;
...
static struct sched_rt_entity *pick_next_rt_entity(struct rt_rq *rt_rq)
{
	...
	while (true) {
		...
		trace_printk("looking in prio %d\n", idx);
		...
		list_for_each_entry(next, queue, run_list) {
			...
			trace_printk("Found next %p\n", (void *)next);
			return next; /* Found. */
		}

		trace_printk("count not find in prio %d, all child groups in this prio must be throttled.\n", idx);
