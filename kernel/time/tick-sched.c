...
static void tick_nohz_stop_tick(struct tick_sched *ts, int cpu)
{
	...
	if (unlikely(expires == KTIME_MAX)) {
		if (ts->nohz_mode == NOHZ_MODE_HIGHRES) {
			...
			trace_tick_event_program(TPS("CancelFromStopTick"), 0, 0, 1, TPS("lowres"));
...
static void tick_nohz_handler(struct clock_event_device *dev)
{
	...
	ktime_t now = ktime_get();
	unsigned long long old_expires;
	...
	if (unlikely(ts->tick_stopped)) {
		 ...
		 */
		trace_tick_event_program(TPS("CancelFromHandler"), 0, 0, 1, TPS("lowres"));
		tick_program_event(KTIME_MAX, 1);
		return;
	}

	old_expires = hrtimer_get_expires(&ts->sched_timer);
