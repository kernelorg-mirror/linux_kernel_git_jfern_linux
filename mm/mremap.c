 ...
 */
bool check_addr_in_prev(struct vm_area_struct *vma, unsigned long addr,
						unsigned long mask)
{
	int addr_masked = addr & mask;
	struct vm_area_struct *prev = NULL, *cur = NULL;

	/* If the address is within the vma, there is no prev mapping of concern. */
	...
	/* The aligned address fell within a previous mapping. */
...
/* Opportunistically realign to PMD/PUD for faster copy. */
